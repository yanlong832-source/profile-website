...
function ProjectCard({ project }: { project: Project }) {
  return <article className={`project-card project-card--${project.accent}`}><div className="project-visual">{project.image ? <img className="project-preview" src={project.image} alt={`${project.name} project preview`} loading="lazy" /> : <div className="project-placeholder" aria-label={`${project.name} preview`}><span>{project.name.slice(0, 2).toUpperCase()}</span><small>PROJECT PREVIEW</small></div>}<div className="project-visual-top"><span className="project-number">{String(projects.indexOf(project) + 1).padStart(2, "0")}</span><span className={`project-status ${project.status}`}>● {project.status === "online" ? "online" : "building"}</span></div><div className="project-visual-bottom"><span className="project-kicker">{project.tags[0] || "PROJECT"}</span><span className="project-open" aria-hidden="true">↗</span></div></div><div className="project-card-body"><div className="project-card-head"><h3>{project.name}</h3></div><p>{project.description}</p><div className="stack">{project.stack.map((item) => <span key={item}>{item}</span>)}</div><div className="project-card-links">{project.url && <a href={project.url} target="_blank" rel="noreferrer">Live Demo <Arrow /></a>}{project.repository && <a href={project.repository} target="_blank" rel="noreferrer">Source Code <span className="source-icon">◈</span></a>}</div></div></article>;
}
